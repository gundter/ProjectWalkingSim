...
must_haves:
  ...
  artifacts:
    - path: "Source/ProjectWalkingSim/Public/Interaction/PickupActor.h"
      provides: "PickupActor with inventory integration, CanInteract check, InitFromItemData"
...
Purpose: PickupActor was a Phase 1 placeholder that just logs and destroys. Now it calls InventoryComponent::TryAddItem, checks IsFull for the interaction prompt, and supports InitFromItemData for spawning discarded items. DoorActor gets optional locked state requiring a specific key item that is consumed on unlock.
...
<task type="auto">
  ...
  <action>
    Modify APickupActor to integrate with the inventory system.

    **Header changes (PickupActor.h):**
    - Add forward declaration: `class UItemDataAsset;`
    ...
    - Add public method:
      ...
      -- Used by InventoryComponent::DiscardItem to configure a freshly spawned pickup
    - Change ItemId and Quantity from protected to public (needed for DiscardItem to set them directly as a fallback). Actually, better to keep protected and use InitFromItemData. Keep them protected.
    - Add private transient member:
      `bool bInventoryFullOnLastCheck = false;` -- cached state for text override

    **Implementation changes (PickupActor.cpp):**
    - Add includes: `Inventory/InventoryComponent.h`, `Inventory/ItemDataAsset.h`
    ...
    - `GetInteractionText_Implementation`:
      - This is a const function. We need the full check here. Get the player pawn from the world, find their InventoryComponent, check IsFull. If full and item can't be stacked into existing slots, return "Inventory Full". Otherwise return base InteractionText ("Pick Up").
      - Actually, simpler: override to check bInventoryFullOnLastCheck (set during CanInteract which runs every tick from InteractionComponent). If true, return NSLOCTEXT("Interaction", "InventoryFull", "Inventory Full"). Otherwise return Super (InteractionText).
      - NOTE: bInventoryFullOnLastCheck must be mutable since CanInteract is const. Make it `mutable bool bInventoryFullOnLastCheck = false;`
  ...
  <done>
    PickupActor::OnInteract calls TryAddItem and destroys only on success. CanInteract tracks inventory-full state. GetInteractionText returns "Inventory Full" when appropriate. InitFromItemData configures a spawned pickup's mesh and properties. Compiles cleanly.
  </done>
</task>
...
- PickupActor::InitFromItemData sets ItemId, Quantity, and loads WorldMesh
