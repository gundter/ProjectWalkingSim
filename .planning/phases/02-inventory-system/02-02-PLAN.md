...
<task type="auto">
  ...
  <action>
    ...
    **InventorySlotWidget.h / .cpp:**
    ...
    - Include InventoryTypes.h (needed for FInventorySlot by-value usage)
    - Forward-declare UItemDataAsset
  ...
  <action>
    ...
    **InventoryWidget.h:**
    ...
    - BindWidget properties (protected):
      ...
      - `TObjectPtr<UItemTooltipWidget> TooltipWidget` (meta = (BindWidget)) -- tooltip placed as child in UMG Blueprint, C++ shows/hides and sets data
    - Editable class reference (protected, EditDefaultsOnly):
    ...
    - Delegate forwarding (the widget re-broadcasts tooltip actions so the HUD can route them):
      ...
      - `FOnTooltipAction OnCombineRequested` (BlueprintAssignable) -- note: this single-param delegate is for the tooltip Combine *button click* (enters combine mode); the two-param combine slot selection delegate is added in Plan 05
    ...
    **InventoryWidget.cpp:**
    - NativeConstruct:
      ...
      4. Bind tooltip's action delegates to internal handlers that re-broadcast through the InventoryWidget's own delegates. Specifically:
         - TooltipWidget->OnUseClicked -> re-broadcast as OnUseRequested
         - TooltipWidget->OnCombineClicked -> re-broadcast as OnCombineRequested
         - TooltipWidget->OnDiscardClicked -> re-broadcast as OnDiscardRequested
