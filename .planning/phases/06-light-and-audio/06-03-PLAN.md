...
- `void BindToWendigoCharacter(AWendigoCharacter* Wendigo)` -- public binding method for external callers (e.g., spawn logic)
...
- `void BindToWendigo()` -- finds Wendigo (owner-first, then world search) and binds to SuspicionComponent delegate
...
BindToWendigo():
**IMPORTANT: Owner-first binding pattern.** Plan 05 places MusicTensionSystem as a default subobject on WendigoCharacter, so GetOwner() IS the Wendigo.
1. First, try to cast the owner: `AWendigoCharacter* Wendigo = Cast<AWendigoCharacter>(GetOwner())`
2. If owner is not a Wendigo (component placed on a different actor), fall back to world search: `UGameplayStatics::GetAllActorsOfClass` to find AWendigoCharacter
3. If Wendigo found by either method, call `BindToWendigoCharacter(Wendigo)` (the public method)
4. If no Wendigo found, log warning (acceptable -- Wendigo may spawn later)

BindToWendigoCharacter(AWendigoCharacter* Wendigo):
1. Get SuspicionComponent from Wendigo, bind OnAlertLevelChanged.AddDynamic
2. Bind to Wendigo->OnBehaviorStateChanged for stinger triggers:
...
4. BindToWendigo() tries Cast<AWendigoCharacter>(GetOwner()) before GetAllActorsOfClass
  </verify>
  <done>MusicTensionSystem crossfades 3 music layers driven by AlertLevel, plays stingers on behavior state transitions, binds to owner Wendigo first (fallback to world search), all via delegate binding (no Tick).</done>
</task>
...
4. MusicTensionSystem::BindToWendigo() uses owner-first cast before world search
5. AmbientAudioManager is an actor placeable in levels
6. Both systems are timer/delegate-driven (no Tick)
7. Predator silence API is callable from Blueprint
</verification>

<success_criteria>
- MusicTensionSystem.h/.cpp compile with 3-layer crossfade driven by alert level
- BindToWendigo() tries Cast<AWendigoCharacter>(GetOwner()) before GetAllActorsOfClass fallback
